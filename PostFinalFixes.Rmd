...
mask2 <- grepl("18F20|1A50", unmatched$Beat)
mask3 <- grepl("redo", unmatched$status)
mask <- mask1&mask2&mask3
sum(mask)
unmatched$Address[mask] <- str_replace(unmatched$Address[mask], "W LOOP S FWY ,", 
                                                    "W LOOP N FWY ,")
...
#  ----------  Convert some S Loop E to South Fwy (oopsie)

mask1 <- grepl("S LOOP E", unmatched$Address)
sum(mask1)
# addresses > 8500
mask2 <- as.integer(str_extract(unmatched$Address, "\\d+"))>=8600
sum(mask2)
mask <- mask1&mask2
sum(mask)

unmatched[mask,]$Address  <- str_replace(unmatched[mask,]$Address, "S LOOP E", "SOUTH")
unmatched[mask,]$geocode_status  <- "redo"
unmatched[mask,]$status  <- "redo"
...
mask1 <- grepl("W BELLFORT AVE", unmatched$Address)
sum(mask1)
mask2 <- grepl("manual", unmatched$geocode_status)
sum(mask1&!mask2)
...
unmatched$Address <- str_replace(unmatched$Address, "PKWT", "PKWY")
unmatched$Address <- str_replace(unmatched$Address, "LOOP W ,", "LOOP W FWY ,")
unmatched$Address <- str_replace(unmatched$Address, "LOOP S ,", "LOOP S FWY ,")
unmatched$Address <- str_replace(unmatched$Address, "9N", "9 N")
...
WSamS <- Fwy_interpolate(unmatched, "W SAM HOUSTON PKWY S", 150, 12950)
Gulf <- Fwy_interpolate(unmatched, "GULF FWY", 450, 21650)
SouthFwy <- Fwy_interpolate(unmatched, "SOUTH FWY", 2350, 13750)
SWFwy <- Fwy_interpolate(unmatched, "SOUTHWEST FWY", 150, 11650)
...
WLoopS <- Fwy_interpolate(unmatched, "W LOOP S FWY", 1150, 9450)
WLoopN <- Fwy_interpolate(unmatched, "W LOOP N FWY", 250, 2450)
SLoopE <- Fwy_interpolate(unmatched, "S LOOP E FWY", 3350, 8450)
SLoopW <- Fwy_interpolate(unmatched, "S LOOP W FWY", 550, 4550)
ELoopN <- Fwy_interpolate(unmatched, "E LOOP N FWY", 250, 3150)
EastFwy <- Fwy_interpolate(unmatched, "EAST FWY", 250, 16250)
Kingwood <- Fwy_interpolate(unmatched, "KINGWOOD DR", 250, 6450)
...
WLakeHouPkwyb <- Fwy_interpolate(unmatched, "W LAKE HOUSTON PKWY", 12650, 16650)
...
generated <- bind_rows(katy, northfwy, eastex, NSamE, SSamE,
                       ...
                       WLoopS, SLoopE, SLoopW, ELoopN, WLoopN, EastFwy,
...
specific <- tribble(
  ...
  "9500-9599", "W LOOP S", "9550", 29.680154, -95.458928, "Houston, TX 77096",
  ...
  "300-399", "S LOOP W", "350", 29.680560, -95.382131, "Houston, TX 77054",
  ...
  "13800-13899", "SOUTH FWY", "13850", 29.593358, -95.386583, "Houston, TX 77047",
  ...
  "0-99", "SOUTHWEST FWY", "50", 29.737811, -95.382206, "Houston, TX 77004",
  ...
  "6500-6599", "KINGWOOD DR", "6550", 30.065553, -95.153076, "Houston, TX 77339",
  ...
  "16400-16499", "W LAKE HOUSTON PKWY", "16450", 29.961614, -95.168542, "Houston, TX 77339",
  "16700-16799", "W LAKE HOUSTON PKWY", "16750", 29.966301, -95.169486, "Houston, TX 77339",
...
saveRDS(unmatched, "~/Dropbox/CrimeStats/GeoTable_19Aug2019.rds")
...
###############################################################
#############   getlatlong with declared zipcode
###############################################################
getlatlong <- function(addr, zip, newaddr){
...
for (i in begin:nrow(unmatched)) {
  if (i==begin){starttime <- now()}
#for (i in begin:(begin+50)) {
  if (unmatched[i,]$status=="redo"){ # only the redo's
    # only do selected cases
    #if (!any(str_detect(unmatched[i,]$Address, onlys))){next()}
    trys <- trys + 1
    print(paste("--->",i,unmatched[i,1], 
    ...
    latlong <- getlatlong(unmatched[i,]$Address, "77546", newaddr)
    if (latlong$status=="success") {
...
##  add in generated points

```{r add generated points}

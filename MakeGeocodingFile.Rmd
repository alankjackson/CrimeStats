...
unmatched$Address <- str_replace(unmatched$Address, "9\\([A-Z]\\)", "9 \\1")
...
for (i in begin:nrow(unmatched)) {
  ...
  if(grepl("redo", unmatched[i,]$geocode_status)){ 
...
sort(unique(unmatched$status))

# Save file

readRDS(unmatched, "~/Dropbox/CrimeStats/GeoTable_22Aug2019_withgen.rds")

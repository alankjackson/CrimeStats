...
district <- "5f"  # South Central Patrol Division
beats <- c("5f10","5f20","5f30","5f40")
...
for (yr in year_range)
{
  for (mon in months)
    ...
    # Feb '14 at district is missing, so skip
    if ((yr == "14") && (mon == "feb")) {next}
...
##---new---##
saveRDS(df, file="~/Dropbox/CrimeStats/District5fRawData.rds")
...
df2$Street[grepl("^DALLAS LOVE FIELD AIRPORT$",df2$Street)] <- NA 
...
dictionary <- c(
                ...
                "BLV 2703",     "BLVD",
                " 35 1/2",      "",
                "HOLLISTEROB",  "HOLLISTER",
                " EN$",         "",
                " UNT$",         "",
                "([A-C,EFG,I-L,N-R,T-X,Z]) \\d{2,6}$",   "\\1"
)
dict <- Makedict(dictionary)
#   test them first
testregex(df2, "Street", dict)
...
#   Apply
df2 <- applyregex(df2, "Street", dict)
df2$Street[grepl("^PR$",df2$Street)] <- NA 
...
df2$Street <- str_replace(df2$Street, "WEST LOOP SOUTH LOOP","W LOOP")
sort(unique(df2$Street[maskloop]))
...
maskloop <- grepl("^LOOP$", df2$Street)
df2$Street[masksuffix&maskloop] <- "N LOOP"
masksuffix <- grepl("W", df2$Suffix)
maskloop <- grepl("^LOOP$", df2$Street)
df2$Street[masksuffix&maskloop] <- "W LOOP"
masksuffix <- grepl("S", df2$Suffix)
maskloop <- grepl("^LOOP$", df2$Street)
df2$Street[masksuffix&maskloop] <- "S LOOP"
masksuffix <- grepl("E", df2$Suffix)
maskloop <- grepl("^LOOP$", df2$Street)
df2$Street[masksuffix&maskloop] <- "E LOOP"
...
dictionary <- c(
                ...
                "W SAM HOUSTON$","W SAM HOUSTON PKWY"
                ) 
dict <- Makedict(dictionary)
#   test them first
testregex(df2, "Street", dict)

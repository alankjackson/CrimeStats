...
ptab <- read.csv("~/mirrors/ajackson/crime/data/ptable.csv")
...
#     April & May 2010 district files missing, so read individual beat files instead

```{r read in April, May 2010 by beat}
keepdf2 <- df2
for (beat in beats)
{ 
  for (mon in c("apr","may"))
    {
    
    url = paste("http://www.houstontx.gov/police/cs/stats2010/",mon,"10/",mon,"10",beat,".htm", sep="")
    print(url)
    ...
    # replace premise codes with premise descriptions
      tempfile["Premise"] <- left_join(tempfile,premise, by="Premise")["Premise Description"]
    # append to df2
    df2 <- bind_rows(df2, tempfile)
  }
...
##---new---##
saveRDS(df2, file="~/Dropbox/CrimeStats/District20gRawData.rds")
...
#  if (!grepl(paste("\\b",a[i],"\\b",sep=""), as.character(ptab[,1]))){
...
#   test them first
testregex(df2, "Street", dict)
df2$Street <- str_replace(df2$Street," CMI BOX 80014", "")
...
#   Apply
df2 <- applyregex(df2, "Street", dict)
df2$Street[grepl("^10226$",df2$Street)] <- NA
df2$Street[grepl("^529$",df2$Street)] <- NA
...
dictionary <- c(
                " BLVD \\w*$",     "BLVD",
                "AV DAS AMERICAS 4200",    "AVENIDA DE LAS AMERICAS",
                " BLOCO .*$",    "",
                " ENT$",           "",
                "LAQUINTA",         "LA QUINTA",
                " 102$",        "",
...
dictionary <- c(
                " STE$",         "",
                "CITY CENTRE PVT",  "CITYCENTRE WAY",
                "COUNTY ROAD",         "CR",
                " DR \\w*$",     " DR",
                "C ZUILAND",      "ZUILAND",
                " A298",       "",
...
#   Apply
df2 <- applyregex(df2, "Street", dict)
df2$Street[grepl("^FORT BEND COUNTY CAC$",df2$Street)] <- NA 
...
#   Apply
df2 <- applyregex(df2, "Street", dict)
df2$Street[grepl("^LIMITED PKKWY$",df2$Street)] <- NA 
...
dictionary <- c(
      "WAY 700", "WAY",
      " DRIVE$", " DR",
      "NELDRIDGE", "N ELDRIDGE",
      "NELKIN", "N ELKIN",
      "NORTH LOOP WEST LOOP", "WEST LOOP",
      "^NORTH ([A-E,G-Z])", "N \\1",
      "NSH 6", "N SH 6"
)
dict <- Makedict(dictionary)
...
dictionary <- c(
      " STE K1", "",
      " RM$", " RD",
      "WEST SAM HOUSTON", "W SAM HOUSTON",

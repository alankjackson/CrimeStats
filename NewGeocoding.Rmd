...
getlatlong <- function(addr){
  ...
  loc1 <- Census_decoder(paste(adr1, street), city, state, "77000")
  j=0
  while (loc1[1] != "success" && j<=4) {
    adr1 <- as.character(as.numeric((adr1))+5)
    loc1 <- Census_decoder(paste(adr1, street), city, state, "77000")
    j=j+1
  }
  loc2 <- Census_decoder(paste(adr2, street), city, state, "77000")
  j=0
  while (loc2[1] != "success" && j<=4) {
...
for (i in 2:2) {
  print(paste("--->",i,newunmatched[i,1]))
  if (grepl("UNK ", newunmatched[i,1])){next}
  oldaddress <- newunmatched[i,1]
  latlong <- getlatlong(oldaddress)
...
newunmatched$Address <- str_replace(newunmatched$Address, "FWY FWY", "FWY")
newunmatched$oldaddress <- str_replace(newunmatched$oldaddress, "FWY SER", "FWY")
newunmatched$oldaddress <- str_replace(newunmatched$oldaddress, "GREENBRIAR DR", "GREENBRIAR ST")
newunmatched$oldaddress <- str_replace(newunmatched$oldaddress, "GREENWICH PL ", "GREENWICH PLACE ")
newunmatched$oldaddress <- str_replace(newunmatched$oldaddress, "GREENWICH PLACE ", "GREENWICH PLACE DR ")

...
getlatlong <- function(addr){
  ...
  newstreet <- trimws(sub("^[0-9]+-[0-9]+", "", x[1]))
  if (street != newstreet) {
    return(data.frame(
      status="failmatch",
      matching_address=loc1$match_address,
      lat=NA,
      long=NA,
      tract=NA,
      block=NA,

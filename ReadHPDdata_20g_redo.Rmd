...
maskaddr <- grepl("^\\d{3,5}\\s",df20$Street)
maskblk <- is.na(df20$Block_Range)
sum(maskaddr&maskblk)
addrs <- str_extract(df20$Street[maskaddr&maskblk],"^\\d{3,5}\\s")
addrs <- as.integer((as.integer(addrs)/100))*100
addrs <- paste(as.character(addrs), as.character(addrs+99),sep="-")
df20$Block_Range[maskaddr&maskblk] <- addrs
df20$Street <- str_replace_all(df20$Street,"^\\d{3,}\\s","")
...
maska <- grepl(matchstring, df20$Street)
matched_values <- str_extract(df20$Street[maska],matchstring)
df20$Type[maska] <- matched_values  #  put value into Type field
df20$Street[maska] <- sub(matchstring, "", df20$Street[maska])  #  remove value from Street field
df20$Street[grepl("^$",df20$Street)] <- NA 
...
df20$Street <- str_replace(df20$Street, "WEST LOOP SOUTH LOOP","WEST LOOP")
sort(unique(df20$Street[maskloop]))
...
df20$Street[grepl("^[NSEW] NA$",df20$Street)] <- NA
keepdf20 <- df20

...
Feb 2014 at District level is repeated from 15e, not 16e. 
```{r read in Feb 2015 by beat}
for (mm in c("feb")) {
for (beat in beats)
    {
    
    url = paste("http://www.houstontx.gov/police/cs/stats2015/",mm,"15/",mm,"15",beat,".htm", sep="")
    print(url)
...
df2$Beat <- str_replace_all(df2$Beat,"g","")
...
df2$Type <- str_replace_all(df2$Type,"^WALK$","") 
...
#   fix up loop 610
maskloop <- grepl("S", df2$Suffix)
maskih <- grepl("^IH 610", df2$Street)
df2$Street[maskloop&maskih] <- "SOUTH LOOP"
maskih <- grepl("^LOOP$", df2$Street)
df2$Street[maskloop&maskih] <- "SOUTH LOOP"
dictionary <- c(
                "^LOOP$",    "SOUTH LOOP",
                "^LOOP W[EST]*", "SOUTH LOOP",
                " 217", "",
                " BUILDING", "",
                "HIGHWAY", "HWY",
                "FOX FIELD", "FOXFIELD"

...
reclean <- c("District10hReCleanData.rds" 
             )
...
# edit addresses for spelling errors
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "2000 W Memorial Loop Dr, Houston, TX 77007, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.774177
  unmatched[mask,]$long   <- -95.442344
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}

mask <- grepl("9 MEMORIAL LOOP", unmatched$Address)

if (sum(mask)>0){
  unmatched[mask,]$match_address <- "1000 Memorial Loop Dr, Houston, TX 77007, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.775845
  unmatched[mask,]$long   <- -95.431376
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "6000 N Picnic Ln, Houston, TX 77007, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.76546
  unmatched[mask,]$long   <- -95.4448
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "500 S Picnic Ln, Houston, TX 77007, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.763081
  unmatched[mask,]$long   <- -95.442344
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "7500 Memorial Dr, Houston, TX 77007, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.766523
  unmatched[mask,]$long   <- -95.442661
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "150 E Memorial Loop Dr, Houston, TX 77007, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.764794
  unmatched[mask,]$long   <- -95.431154
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "6850 Memorial Dr, Houston, TX 77007, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.765306
  unmatched[mask,]$long   <- -95.432661
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "2450 Katy Fwy, Houston, TX 77007, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.776653
  unmatched[mask,]$long   <- -95.386089
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "6200 Hermann Park Dr, Houston, TX 77030, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.715294
  unmatched[mask,]$long   <- -95.389267
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "6000 Hermann Park Dr, Houston, TX 77030, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.719581
  unmatched[mask,]$long   <- -95.388151
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
mask <- grepl("5599 HERMANN PARK DR", unmatched$Address) 

if (sum(mask)>0){
  unmatched[mask,]$match_address <- "5555 Hermann Park Dr, Houston, TX 77030, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.721264
  unmatched[mask,]$long   <- -95.389820
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
if (sum(mask)>0){
  unmatched[mask,]$match_address <- "6000 Golf Course Dr, Houston, TX 77084, USA"
  unmatched[mask,]$status    <- "success"
  unmatched[mask,]$geocode_status    <- "google"
  unmatched[mask,]$lat    <- 29.832923
  unmatched[mask,]$long   <- -95.637641
  unmatched[mask,]$GType    <- "RANGE_INTERPOLATED manual"
}
...
skips <- c("S HEIGHTS", "FWY", "HOMELESS", "PO BOX", "POBOX","WASHINGTON", "HAYS", "POST OAK")

onlys <- c("WHEELER ST", "SAINT CHARLES" ,"SMITH")

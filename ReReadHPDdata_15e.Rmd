...
district <- "15e"  # Midwest Patrol Division
beats <- c("15e10","15e20","15e30","15e40")
...
```{r read in June 2011 by beat}
#   also June 2011
for (mm in c("jun")) {
for (beat in beats)
    {
    
    url = paste("http://www.houstontx.gov/police/cs/stats2011/",mm,"11/",mm,"11",beat,".htm", sep="")
    print(url)
...
#   fix up loop 610
maskloop <- grepl("W", df2$Suffix)
maskih <- grepl("^IH 610", df2$Street)
df2$Street[maskloop&maskih] <- "WEST LOOP"
...
tail(head(sort(unique(df2$Street)),100),100)
dictionary <- c(
                "MAIN AND KIRBY","MAIN @ KIRBY",
                "PEPPER DINE", "PEPPERDINE"
                ) 
dict <- Makedict(dictionary)
#   test them first
testregex(df2, "Street", dict)
